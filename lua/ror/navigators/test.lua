...
function M.visit(mode)
  ...
  if string.match(current_relative_file_path, 'app/models') then
    ...
    elseif #parsed_tests == 1 then
      ...
      if nvim_notify_ok then
        nvim_notify('No test with name: ' .. test_name, vim.log.levels.ERROR, { title = 'Test file not found', timeout = 2500 })
      else
  ...
  elseif string.match(current_relative_file_path, 'app/controllers') then
    ...
    elseif #parsed_tests == 1 then
      ...
      local filename = 'test/controllers/' .. test_name

      vim.ui.select({ 'yes', 'no' }, { prompt = 'Test file not found, create it?' }, function(confirm)
        if confirm == 'yes' then
          vim.cmd(':e ' .. filename)
        end
      end)
